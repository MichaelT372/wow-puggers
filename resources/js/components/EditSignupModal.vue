<template>
  ...
  >
    <template v-slot:header>
      <h5 class="modal-title">Edit signup</h5>
    </template>
    <form ref="form">
...
<script>
  ...
  export default {
    ...
    watch: {
      'signup.class'(val) {
        if (!this.classSpecs.includes(this.signup.spec)) {
    ...
    methods: {
      submit() {
        ...
        if (isValid) {
          ...
          this.$store.dispatch('updateRaider', this.signup)
            .then(() => {
      ...
      closeModal() {
        this.$store.commit('showEditSignupModal', false);
      }
    },

    created() {
      //when signupToEdit is updated in the store, we create a clone of it here for editing
      this.unwatch = this.$store.watch((state) => state.signupToEdit, (value) => {
        this.signup = JSON.parse(JSON.stringify(value));
      })
    },
