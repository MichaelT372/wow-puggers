...
class RaidSeeder extends Seeder
{
    ...
    public function run()
    {
        factory(Raid::class, 3)->create()->each(function (Raid $raid){
            for ($i = 0; $i < 50; $i++) {
                $raid->raiders()->create(
                    factory(RaidSignup::class)->make([
                        'soft_reserve' => array_rand(array_flip($this->softReserveItems[$raid->location]))
                    ])->toArray()
                );
            }
        });
